...
public class NTKParser implements XMLParser {
    ...
    private static final Logger logger = LogManager.getLogger(NTKParser.class);

    /**
     * Instanciation du parser XML.
     * @param filepath Chemin vers le fichier XML à parser
     */
    public NTKParser(String filepath) {
        this.extracted_figures = new ArrayList<>();
        this.keywords = new NTKSemantics().keywords;
    ...
    */
    ...
    private Point parse_point(Element elem, String figureName) {
        String latitude = getVal(elem, NTKKey.PT_LAT);
        String longitude = getVal(elem, NTKKey.PT_LON);
    ...
    private Line parse_line(Element elem, String figureName) {
        NodeList points = elem.getElementsByTagName(this.keywords.get(NTKKey.FIG_POINT));
    ...
    private Ellipse parse_ellipse(Element elem, String figureName) {
        ...
        double angle = Double.parseDouble(getVal(elem, NTKKey.ANGLE));
    ...
    private Circle parse_circle(Element elem, String figureName) {
        Point center = parse_point(elem, "Center");
        String radius = getVal(elem, NTKKey.FIG_HORIZ);
    ...
    private Bullseye parse_bullseye(Element element, String figureName) {
        ...
        double[] horizVert = getHorizVert(element);
        int nbRings = Integer.parseInt(getVal(element, NTKKey.BULLS_RINGS));
        double dist = Double.parseDouble(getVal(element, NTKKey.BULLS_DIST));
    ...
    private Corridor parse_corridor(Element element, String figureName) {
        NodeList points = element.getElementsByTagName(this.keywords.get(NTKKey.FIG_POINT));
        Point start = parse_point((Element) points.item(0), "CorrStart");
        Point end = parse_point((Element) points.item(1), "CorrEnd");
        double width = Double.parseDouble(getVal(element, NTKKey.FIG_HORIZ));
    ...
    private double[] getHorizVert(Element element) {
        String horizontal = getVal(element, NTKKey.FIG_HORIZ);
        String vertical = getVal(element, NTKKey.FIG_VERT);
     ...
     * @see NTKKey
     */
    private String getVal(Element elem, NTKKey key) throws RuntimeException {
        try {
            return elem.getElementsByTagName(this.keywords.get(key)).item(0).getTextContent();
        } catch (NullPointerException e) {
            String figName = elem.getElementsByTagName(this.keywords.get(NTKKey.FIG_NAME)).item(0).getTextContent();
